...
'''
import csv
from os import linesep, posix_fadvise
from matplotlib import pyplot as plt

########################################################################################
def lee_poblaciones(fichero: str) -> list:
    ...
    with open(fichero, "r") as file:
        for x in file.readlines():
            ...
            line = (str(line[0]), str(line[1]), int(line[2]), float(line[3]))
            populations.append(line)
...
def filtra_por_paises_y_anyo(poblaciones, anyo, paises):
    ...
    for x in poblaciones:
        if x[2] == anyo and x[0] in paises:
            filteredList.append((x[0], x[3]))
...
def muestra_evolucion_poblacion(poblaciones, pais):
    ...
    anyos = []
    habitantes = []
    filteredData = filtra_por_pais(poblaciones, pais)

    for x in filteredData:
        anyos.append(x[0])
        habitantes.append(x[1])
    
    plt.title(pais)
    plt.plot(anyos, habitantes)
    plt.show()
...
def muestra_comparativa_paises_anyo(poblaciones, anyo, paises):
    ...
    habitants = []
    Countries = []
    filteredData = filtra_por_paises_y_anyo(poblaciones, anyo, paises)# Calculamos la lista de poblaciones del año dado para los paises de la lista

    for x in filteredData:
        Countries.append(x[0])
        habitants.append(x[1])
    
    CountriesString = str(paises).replace("[", "").replace("]", "").replace("'", "")

    plt.title(f"Comparativa de {CountriesString} en {anyo}")
    index = range(len(Countries))
    plt.bar(index, habitants)
